...
const CustomTooltip = ({ active, payload }: any) => {
  ...
  return (
    <div style={{ borderRadius: 16, border: '1px solid rgba(255,255,255,0.1)', backgroundColor: 'rgba(15, 23, 42, 0.9)', backdropFilter: 'blur(8px)', color: '#F8FAFC', padding: '12px', fontSize: 13, boxShadow: '0 10px 25px -5px rgba(0, 0, 0, 0.5)' }}>
      ...
      <div style={{ color: CAP_COLORS[name] ?? '#6366F1', fontWeight: 800, marginTop: 2 }}>{pct}% of Portfolio</div>
      <div style={{ color: '#94A3B8', fontSize: 12, marginTop: 4, fontWeight: 500 }}>{stocks} holding{stocks !== 1 ? 's' : ''}</div>
    </div>
  )
}
...
export function MarketCapAllocationChart() {
  ...
  return (
    <Card
      title={<div className="flex items-center gap-2"><FiBarChart2 className="text-sky-500"/> Market Cap</div>}
      right={
    ...
    >
      {investments.length === 0 ? (
        <div className="grid h-40 place-items-center text-sm font-medium text-slate-500">Add investments to see market cap breakdown.</div>
      ) : (
        <div className="flex flex-col gap-5 pt-2">
          <div className="grid grid-cols-3 gap-3">
            {pills.map(({ cat, pct, stocks }) => (
              <div key={cat} className="rounded-xl p-3 flex flex-col gap-1 backdrop-blur-sm transition-transform hover:-translate-y-0.5"
                style={{ backgroundColor: CAP_BG[cat], border: `1px solid ${CAP_COLORS[cat]}40` }}>
                <span className="text-[10px] font-black uppercase tracking-wider" style={{ color: CAP_COLORS[cat] }}>{cat}</span>
                <span className="text-xl font-black text-slate-900 dark:text-slate-50 tabular-nums">{pct}%</span>
                <span className="text-[11px] font-semibold text-slate-500 dark:text-slate-400">{stocks} asset{stocks !== 1 ? 's' : ''}</span>
              </div>
            ))}
          </div>
          ...
          {chartData.length > 0 && (
            <div className="h-44 w-full">
              <ResponsiveContainer width="100%" height="100%">
                ...
                <BarChart key={chartKey} data={chartData} barCategoryGap="25%" margin={{ top: 10, right: 0, bottom: 0, left: 0 }}>
                  <XAxis dataKey="name" tick={{ fill: '#64748B', fontSize: 11, fontWeight: 600 }} axisLine={false} tickLine={false} dy={5} />
                  <YAxis hide />
                  <Tooltip content={<CustomTooltip />} cursor={{ fill: 'rgba(148,163,184,0.05)', radius: 8 }} />
                  {/* Ensure animation is explicitly active */}
                  <Bar dataKey="value" radius={[6, 6, 0, 0]} isAnimationActive={true} animationDuration={1000} animationEasing="ease-out">
          ...
          <div className="flex flex-col gap-1 border-t border-slate-100 pt-4 dark:border-slate-800/60">
            {chartData.map((e) => (
              <div key={e.name} className="flex items-center justify-between rounded-lg px-2 py-2 transition-colors hover:bg-slate-50/80 dark:hover:bg-slate-800/40 text-sm">
                <div className="flex items-center gap-2.5">
                  <span className="h-2.5 w-2.5 rounded-full shrink-0 shadow-sm" style={{ backgroundColor: CAP_COLORS[e.name] ?? '#94A3B8' }} />
                  <span className="font-semibold text-slate-700 dark:text-slate-300">{e.name}</span>
                  <span className="text-[11px] font-medium text-slate-400">({e.stocks})</span>
                </div>
                <div className="flex items-center gap-3 tabular-nums">
                  <span className="text-slate-500 text-xs font-semibold">{formatINR(e.value)}</span>
                  <span className="font-bold text-sm" style={{ color: CAP_COLORS[e.name] ?? '#94A3B8' }}>{e.pct}%</span>
                </div>
              </div>
            ))}
          </div>
          ...
          {isLoading && pendingCount > 0 && (
            <p className="text-xs font-semibold text-emerald-500 animate-pulse text-center mt-2">{pendingCount} assets still loading metadata…</p>
          )}
        </div>
      )}
    </Card>
