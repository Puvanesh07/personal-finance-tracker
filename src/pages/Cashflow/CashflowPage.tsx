...
import {
  format, subMonths, parse, isValid, addMonths,
...
import {
  FiActivity, FiPlus, FiEdit2, FiTrash2, FiCalendar,
  FiFilter, FiPieChart, FiTrendingUp, FiTrendingDown,
  FiDollarSign, FiCheck, FiChevronDown, FiChevronLeft, FiChevronRight
} from 'react-icons/fi'
...
function getFYOptions() {
  ...
  return Array.from({ length: 5 }).map((_, i) => {
    ...
    return { key: `${start}-${start + 1}`, label: `FY ${start}–${(start + 1).toString().slice(2)}` }
  })
}

// Separate color palettes for Income and Expense charts
const INCOME_COLORS = ['#10b981', '#3b82f6', '#06b6d4', '#8b5cf6', '#a855f7', '#6366f1', '#14b8a6', '#84cc16']
const EXPENSE_COLORS = ['#f43f5e', '#f97316', '#f59e0b', '#eab308', '#ef4444', '#ec4899', '#d946ef', '#8b5cf6']
...
function InvDropdown({
  ...
  const updatePos = useCallback(() => {
    ...
    setPos({ top: r.bottom + 8 + window.scrollY, left: r.left + window.scrollX, width: r.width })
  }, [])
    ...
    <>
      ...
      >
        <div className="flex items-center gap-3">
          <FiFilter className={`transition-colors ${open ? 'text-emerald-400' : 'text-slate-500'}`} />
          <span className="text-slate-200 font-medium">{selected?.label ?? label}</span>
        </div>
        <FiChevronDown className={`transition-transform duration-300 text-slate-500 ${open ? 'rotate-180' : ''}`} />
      </button>
        ...
        >
          <div className="p-1.5 flex flex-col">
            {options.map(opt => (
              ...
              >
                <span>{opt.label}</span>
                {value === opt.key && <FiCheck className="h-4 w-4 shrink-0" />}
              </button>
            ))}
          </div>
        </div>,
...
function CalendarPicker({
  ...
  return (
    <div className="flex flex-col gap-1">
      ...
      >
        <FiCalendar className={`h-4 w-4 shrink-0 transition-colors ${open ? 'text-emerald-400' : 'text-slate-500'}`} />
        <span>{displayLabel}</span>
        <FiChevronDown className={`ml-auto h-3.5 w-3.5 transition-transform duration-300 text-slate-500 ${open ? 'rotate-180' : ''}`} />
      </button>
...
// ── Summary Card ──────────────────────────────────────────────────────────
function SummaryCard({ label, value, sub, icon, colorClass, borderColor }: {
  label: string; value: string; sub?: string
  icon: React.ReactNode; colorClass: string; borderColor: string
}) {
  return (
    <div className={`relative overflow-hidden rounded-2xl border bg-white/80 p-5 shadow-sm backdrop-blur-md transition-all duration-300 hover:-translate-y-0.5 hover:shadow-md dark:bg-slate-900/50 ${borderColor}`}>
      <div className="flex items-start justify-between">
        <div>
          <p className="text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400">{label}</p>
          <p className={`mt-2 text-2xl font-bold tabular-nums tracking-tight ${colorClass}`}>{value}</p>
          {sub && <p className="mt-1 text-xs font-medium text-slate-400 dark:text-slate-500">{sub}</p>}
        </div>
        <div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-xl ${colorClass.replace('text-', 'bg-').replace('600', '100').replace('400', '500/10')} dark:bg-opacity-10`}>
          {icon}
        </div>
      </div>
    </div>
...
export function CashflowPage() {
  ...
  const filteredRows = useMemo(() => {
    ...
    if (filterMode === 'fy') {
      ...
      rows = rows.filter(e => e.date >= `${startYear}-04-01` && e.date <= `${endYear}-03-31`)
  ...
  const incomeByCategory = useMemo(() => {
    ...
    filteredRows.forEach(r => {
      if (r.type === 'income') grouped[r.category] = (grouped[r.category] || 0) + r.amount
    })
  ...
  const expenseByCategory = useMemo(() => {
    const grouped: Record<string, number> = {}
    filteredRows.forEach(r => {
      if (r.type === 'expense') grouped[r.category] = (grouped[r.category] || 0) + r.amount
    })
    return Object.entries(grouped).map(([name, value]) => ({ name, value })).sort((a, b) => b.value - a.value)
  }, [filteredRows])

  const openDeleteModal = (id: string) => { setSelectedDeleteId(id); setDeleteOpen(true) }
  const confirmDelete = () => {
    if (selectedDeleteId) deleteCashflow(selectedDeleteId)
    setDeleteOpen(false); setSelectedDeleteId(null)
  }
    ...
    <div className="flex flex-col gap-6 pb-8">
      ...
      <header className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 rounded-2xl bg-gradient-to-r from-emerald-500/10 via-teal-500/5 to-transparent p-6 border border-emerald-500/20 dark:from-emerald-500/20 dark:via-teal-500/10 dark:border-emerald-500/30 shadow-sm">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="mt-1 text-sm font-medium text-slate-600 dark:text-slate-300">Track your income sources and expenses over time.</p>
          </div>
        </div>
      ...
      <div className="rounded-2xl border border-slate-200/60 dark:border-slate-800/60 bg-white/60 dark:bg-slate-900/40 backdrop-blur-md shadow-sm">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 px-5 py-3.5 border-b border-slate-100 dark:border-slate-800/60 rounded-t-2xl">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-bold text-slate-700 dark:text-slate-200">Filter Period</span>
          </div>
        ...
        <div className="px-5 py-4 flex flex-wrap items-end gap-3 min-h-[76px]">
          ...
          {filterMode === 'custom' && (
            <>
              <CalendarPicker value={customStart} onChange={setCustomStart} label="From" />
              <div className="self-end pb-3">
                <span className="text-sm font-bold text-slate-500 select-none">→</span>
              </div>
              <CalendarPicker value={customEnd} onChange={setCustomEnd} label="To" />
              <div className="ml-auto flex items-center gap-2 rounded-xl border border-emerald-500/20 bg-emerald-500/5 dark:bg-emerald-500/10 px-4 py-2.5 self-end">
                <span className="h-1.5 w-1.5 rounded-full bg-emerald-400 " />
                <span className="text-xs font-bold text-emerald-600 dark:text-emerald-400">
                  {filteredRows.length} transaction{filteredRows.length !== 1 ? 's' : ''}
          ...
          {filterMode === 'all' && (
            <div className="flex items-center gap-3">
              <span className="text-sm font-medium text-slate-400 dark:text-slate-500">Showing all transactions</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <div className="flex flex-col gap-4">
          <SummaryCard
            label="Total Income" value={formatINR(summary.income)}
            icon={<FiTrendingUp className="h-5 w-5 text-emerald-600 dark:text-emerald-400" />}
            colorClass="text-emerald-600 dark:text-emerald-400"
            borderColor="border-emerald-200/60 dark:border-emerald-500/20" />
          <SummaryCard
            label="Total Expenses" value={formatINR(summary.expense)}
            icon={<FiTrendingDown className="h-5 w-5 text-rose-600 dark:text-rose-400" />}
            colorClass="text-rose-600 dark:text-rose-400"
            borderColor="border-rose-200/60 dark:border-rose-500/20" />
          <SummaryCard
            label="Net Savings" value={formatINR(summary.savings)}
            sub={summary.income > 0 ? `${Math.round((summary.savings / summary.income) * 100)}% savings rate` : undefined}
            icon={<FiDollarSign className="h-5 w-5 text-slate-600 dark:text-slate-300" />}
          ...
          {incomeByCategory.length > 0 ? (
            <div className="h-[250px] w-full">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie data={incomeByCategory} dataKey="value" nameKey="name"
                    cx="50%" cy="50%" innerRadius={65} outerRadius={95} paddingAngle={3}>
                    {incomeByCategory.map((_, i) => (
                      <Cell key={`cell-${i}`} fill={INCOME_COLORS[i % INCOME_COLORS.length]} />
                    ))}
                  </Pie>
                  <Tooltip formatter={(val: any) => formatINR(val as number)}
                    contentStyle={{ borderRadius: '12px', border: 'none', boxShadow: '0 10px 25px -5px rgba(0,0,0,0.15)', backgroundColor: 'rgba(15,23,42,0.95)', color: '#f1f5f9' }} />
                ...
                <PieChart>
                  <Pie data={expenseByCategory} dataKey="value" nameKey="name"
                    ...
                    {expenseByCategory.map((_, i) => (
                      <Cell key={`cell-${i}`} fill={EXPENSE_COLORS[i % EXPENSE_COLORS.length]} />
                    ))}
                  </Pie>
                  <Tooltip formatter={(val: any) => formatINR(val as number)}
                    contentStyle={{ borderRadius: '12px', border: 'none', boxShadow: '0 10px 25px -5px rgba(0,0,0,0.15)', backgroundColor: 'rgba(15,23,42,0.95)', color: '#f1f5f9' }} />
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full text-left text-sm whitespace-nowrap">
            <thead className="border-b border-slate-200/60 dark:border-slate-800/60 bg-slate-50/50 dark:bg-slate-800/30">
              <tr>
                {['Date', 'Type', 'Category', 'Notes', 'Amount', 'Actions'].map((h, i) => (
                  <th key={h} className={`px-5 py-4 text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400 ${i === 4 ? 'text-right' : i === 5 ? 'text-center' : ''}`}>
                    {h}
                  </th>
                ))}
              </tr>
              ...
              {filteredRows.length === 0 ? (
                <tr>
                  <td colSpan={6} className="px-5 py-14 text-center">
                    ...
                    <p className="text-sm font-medium text-slate-400">No transactions found for the selected period.</p>
                  </td>
                </tr>
              ) : (
                filteredRows.map(e => (
                  <tr key={e.id} className="group transition-colors hover:bg-slate-50/80 dark:hover:bg-slate-800/40">
                    ...
                    <td className="px-5 py-4 max-w-xs truncate text-slate-500 dark:text-slate-400">{e.notes ?? '—'}</td>
                    ...
                    <td className="px-5 py-4">
                      <div className="flex justify-center gap-2">
                        ...
                        <button type="button" onClick={() => openDeleteModal(e.id)}
      ...
      {edit && <UpsertCashflowModal open={!!edit} onClose={() => setEdit(null)} mode="edit" entry={edit} />}

      <Modal open={deleteOpen} onClose={() => setDeleteOpen(false)} title="⚠ Confirm Deletion">
        <div className="space-y-6">
          <p className="text-sm text-slate-400">This will permanently delete the transaction.</p>
          <div className="flex justify-end gap-3 border-t border-slate-200 dark:border-slate-800 pt-5">
            <button onClick={() => setDeleteOpen(false)}
              className="rounded-xl px-5 py-2.5 text-sm font-bold text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800">Cancel</button>
            <button onClick={confirmDelete}
              className="rounded-xl bg-red-600 px-6 py-2.5 text-sm font-bold text-white hover:bg-red-700">Yes, Delete</button>
          </div>
        </div>
      </Modal>
    </div>
