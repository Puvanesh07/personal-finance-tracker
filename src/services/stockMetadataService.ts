...
import { ETF_MAP, NSE_STOCK_DB, ISIN_TO_SYMBOL, classifyMutualFundByName } from '../data/nseStockdata'

export type MarketCapCategory =
  | 'Large Cap' | 'Mid Cap' | 'Small Cap'
  | 'Large & Mid Cap' | 'Multi Cap' | 'Hybrid' | 'Debt' | 'Unknown'
...
export function classifyMarketCap(marketCap: number | null, staticCap?: string): MarketCapCategory {
  const KNOWN: MarketCapCategory[] = ['Large Cap', 'Mid Cap', 'Small Cap', 'Large & Mid Cap', 'Multi Cap', 'Hybrid', 'Debt']
  if (staticCap && KNOWN.includes(staticCap as MarketCapCategory)) return staticCap as MarketCapCategory
...
function resolveOffline(symbol?: string, isin?: string, name?: string): StockMetadata | null {
  ...
  if (isn) {
    ...
    if (mappedSym) {
      ...
      if (d) return make(mappedSym, d.sector, d.industry, null, d.cap, nm || mappedSym, 'isin_static')
  ...
  if (nm) {
    ...
    if (mf) return make(sym ?? '', mf.sector, mf.sector, null, mf.cap, nm, 'name_classify')
  }
...
function make(
  ...
  marketCap: number | null, staticCap: string, longName: string, source: string
): StockMetadata {
  return {
    symbol, sector, industry, marketCap,
    marketCapCategory: classifyMarketCap(marketCap, staticCap),
    longName: longName || symbol, source, fetchedAt: Date.now(),
  }
}
...
function readCache(key: string): StockMetadata | null {
  ...
  try {
    ...
    if (Date.now() - p.fetchedAt >= TTL_MS) { sessionStorage.removeItem(`smeta_${key}`); return null }
    memCache.set(key, p); return p
  } catch { return null }
}

function writeCache(key: string, meta: StockMetadata) {
  memCache.set(key, meta)
  try { sessionStorage.setItem(`smeta_${key}`, JSON.stringify(meta)) } catch { /* quota */ }
}

// ─── Network fallback — Screener.in via Netlify function ─────────────────────
const inFlight = new Map<string, Promise<StockMetadata>>()

async function fetchFromFunction(symbol: string, isin: string, name: string): Promise<StockMetadata> {
  const p = new URLSearchParams()
  if (symbol) p.set('symbol', symbol)
  if (isin)   p.set('isin', isin)
  if (name)   p.set('name', name)
  const res  = await fetch(`/.netlify/functions/stock-meta?${p}`, { signal: AbortSignal.timeout(15_000) })
...
function networkFetch(ckey: string, symbol: string, isin: string, name: string, fallbackDisplay: string): Promise<StockMetadata> {
  const cached = readCache(ckey)
  if (cached) return Promise.resolve(cached)
  const existing = inFlight.get(ckey)
  if (existing) return existing
  const p = fetchFromFunction(symbol, isin, name)
    .then((m) => { writeCache(ckey, m); return m })
    .catch(() => make(symbol || fallbackDisplay, 'Unknown', 'Unknown', null, '', fallbackDisplay, 'none'))
...
export async function fetchAllMetadata(
  ...
  const BATCH = 3, DELAY = 300
  for (let i = 0; i < pending.length; i += BATCH) {
    const batch = pending.slice(i, i + BATCH)
    const settled = await Promise.allSettled(batch.map((b) => b.promise))
    settled.forEach((s, idx) => {
      const { key } = batch[idx]
      result.set(key, s.status === 'fulfilled'
        ? s.value
        : make(key, 'Unknown', 'Unknown', null, '', key, 'error'))
    })
    if (i + BATCH < pending.length) await new Promise((r) => setTimeout(r, DELAY))
  }
